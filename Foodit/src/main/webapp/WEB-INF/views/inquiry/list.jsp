				...
				.click(
						function() {
							...
							if (confirm("선택한 게시물을 삭제 하시겠습니까 ?")) {
								...
								f.action = "${pageContext.request.contextPath}/inquiry/deleteList.do";
								f.submit();
							}
						});
	});
	...
	<main>
		...
		<form name="listForm" method="post">
				...
				<div class="btn_right" align="center">
					...
					<c:if test="${sessionScope.member.memberId=='admin'}">
						<button type="button" class="btn"
							onclick="location.href='${pageContext.request.contextPath}/inquiry/write.do?size=${size}';">글올리기</button>
					</c:if>
					<button type="button" class="btn"
			...
			<table class="table-container">
				...
				<c:forEach var="dto" items="${listperInquery}">
					<tr>
						<c:if test="${sessionScope.member.memberId=='admin'}">
							<td><input type="checkbox" name="nums"
								value="${dto.inquiryNo}"></td>
						</c:if>
						<td>${dto.inquiryNo}</td>
						<td class="left"><a href="${articleUrl}&num=${dto.inquiryNo}">${dto.subject}</a>
						</td>
						<td>관리자</td>
